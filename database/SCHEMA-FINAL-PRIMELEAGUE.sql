...
CREATE TABLE `discord_links` (
  ...
  `verify_code` VARCHAR(8) NULL DEFAULT NULL,
  `verify_expires_at` TIMESTAMP NULL DEFAULT NULL,
  `is_primary` TINYINT(1) NOT NULL DEFAULT 0,
  `linked_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `verified_at` TIMESTAMP NULL DEFAULT NULL,
  PRIMARY KEY (`discord_id`),
  UNIQUE KEY `uk_discord_links_player_id` (`player_id`),
  KEY `idx_discord_links_verify_code` (`verify_code`),
  KEY `idx_discord_links_expires_at` (`verify_expires_at`),
  KEY `idx_discord_links_player_name` (`player_name`),
...
BEGIN
    ...
    WHERE pd.name = p_player_name 
      AND dl.verify_code = p_verify_code
      AND dl.verify_expires_at > NOW()
