...
import br.com.primeleague.core.database.DataManager;
...
public class SchemaValidator {
    ...
    private boolean validateTableStructures() {
        ...
        // Verificar se a seção tables existe
        if (schemaDefinition.getConfigurationSection("tables") == null) {
            core.getLogger().warning("⚠️ [SchemaValidator] Seção 'tables' não encontrada no schema-definition.yml");
            return true; // Não falhar se não houver tabelas definidas
        }
        ...
        if (tables.isEmpty()) {
            core.getLogger().info("📋 [SchemaValidator] Nenhuma tabela definida para validação");
            return true;
        }
    ...
    private boolean validateDataIntegrity() {
        ...
        boolean allValid = true;
        List<Map<String, Object>> validations = schemaDefinition.getMapList("validations.data_integrity");
